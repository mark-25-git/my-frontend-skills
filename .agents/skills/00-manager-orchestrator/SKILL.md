...
### Gate 0 — Prerequisite
Before opening **any** gate, you MUST verify that `/.agents/product-marketing-context.md` contains actual business data (not just the template). If it matches the template, halt and ask the user to fill it out.
...
4. **Halt only if `blocked: true`** or if you need the user to approve a specific visual design choice.
